...
function requireBinaryGateArity(analysis, label) {
  Object.entries(analysis.circuits).forEach(([circuitName, graph]) => {
    graph.nodes.forEach((current) => {
      if (["AND", "OR", "NAND", "NOR"].includes(current.gate)) {
        require(current.inputs.length === 2, `${label} ${circuitName} contains a non-binary ${current.gate} gate: ${current.inputs.length} inputs.`);
      }
      if (current.gate === "NOT") {
        require(current.inputs.length === 1, `${label} ${circuitName} contains a NOT gate with ${current.inputs.length} inputs.`);
      }
    });
  });
}
...
requireBinaryGateArity(expressionAnalysis, "Expression analysis");
...
requireBinaryGateArity(cAnalysis, "Minterm analysis");
...
requireBinaryGateArity(maxtermAnalysis, "Maxterm analysis");
...
requireBinaryGateArity(constantOne, "Constant-one analysis");
requireBinaryGateArity(constantZero, "Constant-zero analysis");
...
requireBinaryGateArity(dontCareAnalysis, "Don't-care analysis");
...
[2, 3, 4, 5, 6].forEach((count) => {
  ...
  requireBinaryGateArity(analysis, `${count}-variable analysis`);
});
